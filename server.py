...
def check(method, url, body) :
    if ('/' in url) :
        host, path = url.split('/') # ex) 127.0.0.1/index.html => host = 127.0.0.1, path = index.html
        if (host == IP) :
            if (method == 'HEAD') :
                return fillHeaderResp(header[0])  # ex) request = HEAD 127.0.0.1/ => response = 100, continue
            if (path == 'index.html') :
                if (method == 'GET') :
                    return fillHeaderResp(header[1], body) # ex) request = GET 127.0.0.1/index.html => response = 200, ok
                else :
                    return fillHeaderResp(header[3]) # ex) request = HEAD 127.0.0.1/index.html => response = 400, bad request
            elif (path == 'update') :
                if (method == 'PUT') :
                    return respPut(body) # ex) request = PUT 127.0.0.1/update body => resPut(body) 함수를 실행해서 처리
                else :
                    return fillHeaderResp(header[3]) # ex) request = HEAD 127.0.0.1/update => response = 400, bad request
            elif (path == 'create') :
                if (method == 'POST') :
                    return respPost(body) # ex) request = POST 127.0.0.1/create body => resPost(body) 함수를 실행해서 처리
                else :
                    return fillHeaderResp(header[3]) # ex) request = HEAD 127.0.0.1/create => response = 400, bad request
            else :
                return fillHeaderResp(header[4]) # ex) request = POST 127.0.0.1/coding => response = 404, not found
        else :
            return fillHeaderResp(header[3], body="refused connection") # ex) request = 128.0.0.1/index.html => 400, bad request
    else : 
        return fillHeaderResp(header[4]) # ex) request = 127.0.0.1 => response = 404, not found
